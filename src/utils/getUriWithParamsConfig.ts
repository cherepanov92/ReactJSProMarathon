...
function getUriWithParamsConfig(endpointConfig: string, params: any) {
    ...
    const pathname = Object.keys(query).reduce((acc, val) => {
        if (acc.indexOf(`{${val}`) !== -1) {
            const result = acc.replace(`{${val}}`, query[val as typeof query]);
            delete query[val as typeof query];
            return result;
        }
        return acc;
